...
contract CakeV3PoolQuoteHelperV2 {
    struct TickData {
        int24 tick;
        bool initialized;
        int128 liquidityNet;
    ...
    function getTicks(address poolAddress, int24 startTick, int24 stopTick, bool zeroForOne, uint256 maxTickTravel)
        external
        view
        returns (TickData[] memory ticks)
    {
        ...
        if (zeroForOne) {
            require(MIN_TICK <= stopTick && stopTick < startTick, "Invalid tick range");

            int24 currentTick = startTick;
            uint256 leftCount = 0;
            TickData[] memory ticksLeft = new TickData[](maxTickTravel);

            while (currentTick > stopTick && leftCount < maxTickTravel) {
                (int24 nextInitializedTick, bool initialized) =
                    nextInitializedTickWithinOneWord(pool, currentTick, tickSpacing, true);
                ...
                ticksLeft[leftCount] =
                    TickData({tick: nextInitializedTick, initialized: initialized, liquidityNet: liquidityNet});
            ...
            assembly {
                mstore(ticksLeft, leftCount)
            }
            ticks = ticksLeft;
        } else {
            require(startTick < stopTick && stopTick <= MAX_TICK, "Invalid tick range");

            int24 currentTick = startTick;
            uint256 rightCount = 0;
            TickData[] memory ticksRight = new TickData[](maxTickTravel);

            while (currentTick < stopTick && rightCount < maxTickTravel) {
                (int24 nextInitializedTick, bool initialized) =
                    nextInitializedTickWithinOneWord(pool, currentTick, tickSpacing, false);
    ...
    function nextInitializedTickWithinOneWord(IPancakeV3Pool pool, int24 tick, int24 tickSpacing, bool lte)
        private
        view

...
abstract contract UniV2SwapRouter is UniswapImmutables, Permit2Payments {
    error UniV2TooLittleReceived();
    error UniV2TooMuchRequested();
    error UniV2InvalidPath();

    function _uniV2Swap(address[] calldata path, address recipient, address pair) private {
        unchecked {
    ...
    function uniV2SwapExactInput(
        ...
        _uniV2Swap(path, recipient, firstPair);

        uint256 amountOut = tokenOut.balanceOf(recipient) - balanceBefore;
        if (amountOut < amountOutMinimum) revert UniV2TooLittleReceived();
    }
    ...
    function uniV2SwapExactOutput(
        ...
        if (amountIn > amountInMaximum) revert UniV2TooMuchRequested();

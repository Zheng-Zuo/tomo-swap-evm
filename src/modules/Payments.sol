...
import {TransferHelper} from "../libraries/TransferHelper.sol"; // tron's usdt is not compatible with solmate's SafeTransferLib
...
abstract contract Payments is PaymentsImmutables {
    ...
    function sweep(address token, address recipient, uint256 amountMinimum) internal {
        ...
        if (token == Constants.ETH) {
            ...
            if (balance > 0) TransferHelper.safeTransfer(token, recipient, balance);
        }
    }

...
import {IPair} from "../../../interfaces/shadowExchange/IPair.sol";
import {IPairFactory} from "../../../interfaces/shadowExchange/IPairFactory.sol";
...
library RamsesLegacyLibrary {
    ...
    /// @param stable If pair is xy(x^2 + y^2)
    /// @return pair The resultant pair address
    function pairForPreSorted(address factory, bytes32 initCodeHash, address token0, address token1, bool stable)
        private
        pure
        returns (address pair)
    {
        pair = address(
            uint160(
                uint256(
                    keccak256(
                        abi.encodePacked(
                            hex"ff", factory, keccak256(abi.encodePacked(token0, token1, stable)), initCodeHash
                        )
                    )
                )
            )
    ...
    function pairAndReservesFor(address factory, bytes32 initCodeHash, address tokenA, address tokenB, bool stable)
        private
        view
        returns (address pair, uint256 reserveA, uint256 reserveB, uint256 decimalsA, uint256 decimalsB)
    {
        address token0;
        ...
        if (stable) {
            (decimalsA, decimalsB) = tokenA == token0 ? (decimals0, decimals1) : (decimals1, decimals0);
        }
    }
    ...
    function getAmountOut(
        ...
        if (stable) {
            ...
            uint256 y = reserveOut - _get_y(amountIn + reserveIn, k, reserveOut, decimalsIn, decimalsOut);
    ...
    function getAmountIn(
        ...
        if (stable) {
            ...
            uint256 y = _get_y(reserveOut - amountOut, k, reserveIn, decimalsIn, decimalsOut) - reserveIn;
    ...
    function getReserves(address factory, bytes32 initCodeHash, address tokenA, address tokenB, bool stable)
        internal
        view
        returns (uint256 reserveA, uint256 reserveB, uint256 decimalsA, uint256 decimalsB)
    {
        (address token0,) = sortTokens(tokenA, tokenB);
        (uint256 decimals0, uint256 decimals1, uint256 reserve0, uint256 reserve1,,,) =
            IPair(pairFor(factory, initCodeHash, tokenA, tokenB, stable)).metadata();
        (reserveA, reserveB, decimalsA, decimalsB) =
            tokenA == token0 ? (reserve0, reserve1, decimals0, decimals1) : (reserve1, reserve0, decimals1, decimals0);
    }
    ...
    function getAmountInMultihop(
        ...
        for (uint256 i = path.length - 1; i > 0; i--) {
            ...
            uint256 decimalsIn;
            uint256 decimalsOut;
            (pair, reserveIn, reserveOut, decimalsIn, decimalsOut) =
                pairAndReservesFor(factory, initCodeHash, path[i].from, path[i].to, path[i].stable);
            amount = getAmountIn(amount, reserveIn, reserveOut, decimalsIn, decimalsOut, path[i].stable);
    ...
    function _get_y(uint256 x0, uint256 xy, uint256 y, uint256 decimals0, uint256 decimals1)
        internal
        pure
        returns (uint256 _y)
    {

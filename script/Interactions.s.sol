...
// dogeos testnet
contract UniV3SwapDogeTestnet is Script, PermitSignature {
    using SafeERC20 for IERC20;

    IPermit2 constant PERMIT2 = IPermit2(0x5d47029371233B4925824fA0EeF8F3B4195bbac4);
    IERC20 constant USDT = IERC20(0xc622BC0CE453a3321b4A097d762b0c4EEa186d07);
    IERC20 constant USDC = IWETH9(0xd01c252D92554eC19c50Da53348f85117C23c431);
    ...
    function swap(address mostRecentlyDeployed) public {
        ...
        uint256 permit2Allowance = USDC.allowance(signer, address(PERMIT2));
        if (permit2Allowance < swapAmount) {
            vm.startBroadcast(signerPrivateKey);
            USDC.forceApprove(address(PERMIT2), type(uint256).max);
            vm.stopBroadcast();
            console2.log("Successfully approved permit2 for maximum token allowance...");
        ...
        console2.log("Swapped USDTC for USDT through one pool with amount: ", swapAmount);
    }

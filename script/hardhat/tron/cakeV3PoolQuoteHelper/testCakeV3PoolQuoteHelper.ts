...
async function main() {
    ...
    let cakeV3PoolQuoteHelperAddress: string;
    ...
    if (network === "nile") {
        ...
        cakeV3PoolQuoteHelperAddress = "TFTNxbuW2JyBo7VPQrhBraALhDqop73PJD";
        ...
        cakeV3PoolQuoteHelperAddress = "TNQCgKNgmXdX3CbzCxFRTKQWjS8qxKVsDC";
    } else {
        throw new Error("Invalid network");
    }

    const quoteHelperContract = await tronWeb.contract(cakeV3PoolQuoteHelperArtifact.abi, cakeV3PoolQuoteHelperAddress);
    let res = await quoteHelperContract.getPoolState(poolAddress).call();
    ...
    res = await quoteHelperContract.getTicks(poolAddress, quotedTick, -887272, true).call();
    const ticks = res[0];

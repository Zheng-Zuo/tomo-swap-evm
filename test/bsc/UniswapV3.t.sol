...
contract UniswapV3Test is Test, PermitSignature {
    ...
    IERC20 constant BTCB = IERC20(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c); // 500
    ...
    // WBNB -> USDT 100 fee
    function test_singleSwapExactInput() public airdropWbnb {
    ...
    // WBNB -> USDT 100 fee -> BTCB 500 fee
    function test_multiHopSwapExactInput() public airdropWbnb {
        (IAllowanceTransfer.PermitSingle memory permitSingle, bytes memory sig) = _generateSignature(
            user,
            address(WBNB),
            AMOUNT,
        ...
        assertEq(WBNB.balanceOf(user), BALANCE - AMOUNT);
        assertEq(BTCB.balanceOf(user), amountOut);
    }
}

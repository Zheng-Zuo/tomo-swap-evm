...
contract UniswapV3Test is Test, PermitSignature {
    ...
    IERC20 constant USDT = IERC20(0xc2132D05D31c914a87C6611C10748AEb04B58e8F); // 3000
    IERC20 constant USDC = IERC20(0x3c499c542cEF5E3811e1192ce70d8cC03d5c3359); // 100
    ...
    function setUp() public {
        ...
        vm.startPrank(user);
        deal(user, BALANCE);
        WMATIC.approve(address(PERMIT2), type(uint256).max);
    ...
    modifier airdropWeth() {
        deal(address(WMATIC), user, BALANCE);
        _;
    }
    ...
    function test_checkInitialState() public view {
        ...
        uint256 userWethBalance = WMATIC.balanceOf(user);
        ...
        uint256 permit2WethAllowance = WMATIC.allowance(user, address(PERMIT2));
        ...
        (uint160 amount, uint48 expiration, uint48 nonce) = PERMIT2.allowance(user, address(WMATIC), address(router));
        assertEq(amount, 0);
    ...
    function test_singleSwapExactInput() public airdropWeth {
        ...
        assertEq(WMATIC.balanceOf(user), BALANCE - AMOUNT);
        assertEq(USDT.balanceOf(user), amountOut);
    }

    // WMATIC -> USDT 100 fee -> USDC 100 fee
